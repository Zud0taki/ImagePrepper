...
# input handling
for txt in glob.glob(r"C:\Users\KIZwei\Desktop\Problemfall\txt\*"):
    txt_input.append(txt)
for img in glob.glob(r"C:\Users\KIZwei\Desktop\Problemfall\img\*"):
...
for x in range(len(img_input)):
    ...
    export_name = export_name.split("\\txt")
    ...
    if (len(x_max_list) > 0):
        for line in txt_file:
            ...
            if (label_name == "ship"):
                ...
                txt_name = export_name + "/res/" + "t" + str(label_counter) + ".txt"
                txtfile = open(txt_name, 'w')
                for element in export_list:
                    txtfile.write(str(element) + "\n")
                txtfile.close()
                export_list = []
                save = cv.imwrite(export_name + "/res/" + "t" + str(label_counter) + ".tif", cropped)
